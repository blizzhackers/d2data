			...
			[
				...
				this.json.uniqueitems.forEach(unique => {
					if (unique.code === item.code && unique['enabled']) {
						...
						this.items.push({
							...
							type,
							name: name + ' [Unique]',
							searchable: [name, itemname, 'Unique', unique.code, ...this.getKeywords(type.Code), isExceptional ? 'exceptional' : 'non-exceptional', isElite ? 'elite' : 'non-elite'].join(' '),
							tooltip: [
								'Type: ' + itemname,
								'Code: ' + unique.code,
								'Keywords: ' + [name, itemname, 'Unique', unique.code, ...this.getKeywords(type.Code)].join(' '),
							].join('\n'),
							use: false,
							unique,
							func: ratioFuncs,
				...
				this.json.setitems.forEach(set => {
					if (set.item === item.code) {
						...
						this.items.push({
							...
							type,
							name: name + ' [Set]',
							searchable: [name, itemname, 'Set', set.item, ...this.getKeywords(type.Code), isExceptional ? 'exceptional' : 'non-exceptional', isElite ? 'elite' : 'non-elite'].join(' '),
							tooltip: [
								'Type: ' + itemname,
								'Code: ' + set.item,
								'Keywords: ' + [name, itemname, 'Set', set.item, ...this.getKeywords(type.Code)].join(' '),
							].join('\n'),
							use: false,
							set,
							func: ratioFuncs,
				...
				if (type['Rare']) {
					this.items.push({
						...
						type,
						name: itemname + ' [Rare]',
						searchable: [itemname, 'Rare', item.code, ...this.getKeywords(type.Code), isExceptional ? 'exceptional' : 'non-exceptional', isElite ? 'elite' : 'non-elite'].join(' '),
						tooltip: 'Code: ' + item.code + '\nKeywords: ' + [itemname, 'Rare', item.code, ...this.getKeywords(type.Code)].join(' '),
						use: false,
						func: ratioFuncs,
					});
				}
				...
				if (!type['Normal']) {
					this.items.push({
						...
						type,
						name: itemname + ' [Magic]',
						searchable: [itemname, 'Magic', item.code, ...this.getKeywords(type.Code), isExceptional ? 'exceptional' : 'non-exceptional', isElite ? 'elite' : 'non-elite'].join(' '),
						tooltip: 'Code: ' + item.code + '\nKeywords: ' + [itemname, 'Magic', item.code, ...this.getKeywords(type.Code)].join(' '),
						use: false,
						func: ratioFuncs,
					});
				}
				...
				if (!type['Magic'] && !type['Normal']) {
					this.items.push({
						...
						searchable: [itemname, 'Superior', item.code, ...this.getKeywords(type.Code), isExceptional ? 'exceptional' : 'non-exceptional', isElite ? 'elite' : 'non-elite'].join(' '),
						tooltip: 'Code: ' + item.code + '\nKeywords: ' + [itemname, 'Superior', item.code, ...this.getKeywords(type.Code)].join(' '),
						use: false,
						func: ratioFuncs,
					});
				}
				...
				if (!type['Magic']) {
					this.items.push({
						...
						searchable: [itemname, 'Normal', item.code, ...this.getKeywords(type.Code), isExceptional ? 'exceptional' : 'non-exceptional', isElite ? 'elite' : 'non-elite'].join(' '),
						tooltip: 'Code: ' + item.code + '\nKeywords: ' + [itemname, 'Normal', item.code, ...this.getKeywords(type.Code)].join(' '),
						use: false,
						func: ratioFuncs,
					});
				}
				...
				if (!type['Magic'] && !type['Normal']) {
					this.items.push({
						...
						searchable: [itemname, 'Low Quality', item.code, ...this.getKeywords(type.Code), isExceptional ? 'exceptional' : 'non-exceptional', isElite ? 'elite' : 'non-elite'].join(' '),
						tooltip: 'Code: ' + item.code + '\nKeywords: ' + [itemname, 'Low Quality', item.code, ...this.getKeywords(type.Code)].join(' '),
						use: false,
						func: ratioFuncs,
					});
				}
